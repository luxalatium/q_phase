...
void Diosi2d::Evolve()
{
    ...
    double Dqq = (temp == 0.0) ? 0.0 : gamma * hb * hb / (12.0 * m * kb * temp);
    double Dpq = (temp == 0.0) ? 0.0 : gamma * hb * hb * omega / (6.0 * PI * kb * temp);
        ...
        while ( TBL.size() != 0 && !isFullGrid && Excount < ExLimit)
        {
            ...
            if ( isFirstExtrp )  {
                ...
                #pragma omp parallel 
                {
                    ...
                    #pragma omp for private(xx1,xx2,f0,f1p1,f1m1,f2p1,f2m1,f1p2,f1m2,f2p2,f2m2,f1p1f2p1,f1p1f2m1,f1m1f2p1,f1m1f2m1) schedule(runtime)
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            if (TAMask[i1*W1+i2])  {
                                xx1 = Box[0] + i1 * H[0];
                                ...
                                f2p2 = F[i1*W1+(i2+2)];
                                f2m2 = F[i1*W1+(i2-2)];
                                f1p1f2p1 = F[(i1+1)*W1+(i2+1)];
                                f1p1f2m1 = F[(i1+1)*W1+(i2-1)];
                                f1m1f2p1 = F[(i1-1)*W1+(i2+1)];
                                f1m1f2m1 = F[(i1-1)*W1+(i2-1)];
                    ...
                    // RK4-2
                    #pragma omp for private(xx1,xx2,f0,f1p1,f1m1,f2p1,f2m1,f1p2,f1m2,f2p2,f2m2,kk0,kk1p1,kk1m1,kk2p1,kk2m1,kk1p2,kk1m2,kk2p2,kk2m2,f1p1f2p1,f1p1f2m1,f1m1f2p1,f1m1f2m1,kk1p1kk2p1,kk1p1kk2m1,kk1m1kk2p1,kk1m1kk2m1) schedule(runtime)
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            if (TAMask[i1*W1+i2])  {
                                xx1 = Box[0] + i1 * H[0];
                                ...
                                f1m1f2p1 = F[(i1-1)*W1+(i2+1)];
                                f1m1f2m1 = F[(i1-1)*W1+(i2-1)];
                                kk1p1kk2p1 = KK1[(i1+1)*W1+(i2+1)];
                                kk1p1kk2m1 = KK1[(i1+1)*W1+(i2-1)];
                                kk1m1kk2p1 = KK1[(i1-1)*W1+(i2+1)];
                                kk1m1kk2m1 = KK1[(i1-1)*W1+(i2-1)];
                    ...
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            if (TAMask[i1*W1+i2])  {
                                ...
                                f1m1f2p1 = F[(i1-1)*W1+(i2+1)];
                                f1m1f2m1 = F[(i1-1)*W1+(i2-1)];
                                kk1p1kk2p1 = KK2[(i1+1)*W1+(i2+1)];
                                kk1p1kk2m1 = KK2[(i1+1)*W1+(i2-1)];
                                kk1m1kk2p1 = KK2[(i1-1)*W1+(i2+1)];
                                kk1m1kk2m1 = KK2[(i1-1)*W1+(i2-1)];
                    ...
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            if (TAMask[i1*W1+i2])  {
                                ...
                                f1m1f2p1 = F[(i1-1)*W1+(i2+1)];
                                f1m1f2m1 = F[(i1-1)*W1+(i2-1)];
                                kk1p1kk2p1 = KK3[(i1+1)*W1+(i2+1)];
                                kk1p1kk2m1 = KK3[(i1+1)*W1+(i2-1)];
                                kk1m1kk2p1 = KK3[(i1-1)*W1+(i2+1)];
                                kk1m1kk2m1 = KK3[(i1-1)*W1+(i2-1)];
            ...
            else  
            {
                ...
                // RK4-1
                #pragma omp parallel for private(g1,g2,xx1,xx2,f0,f1p1,f1m1,f1p2,f1m2,f2p1,f2m1,f2p2,f2m2,f1p1f2p1,f1p1f2m1,f1m1f2p1,f1m1f2m1)
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f2p2 = F[g1*W1+(g2+2)];
                    f2m2 = F[g1*W1+(g2-2)];
                    f1p1f2p1 = F[(g1+1)*W1+(g2+1)];
                    f1p1f2m1 = F[(g1+1)*W1+(g2-1)];
                    f1m1f2p1 = F[(g1-1)*W1+(g2+1)];
                    f1m1f2m1 = F[(g1-1)*W1+(g2-1)];
                ...
                // RK4-2
                #pragma omp parallel for private(g1,g2,xx1,xx2,f0,f1p1,f1m1,f2p1,f2m1,f1p2,f1m2,f2p2,f2m2,kk0,kk1p1,kk1m1,kk2p1,kk2m1,kk1p2,kk1m2,kk2p2,kk2m2,f1p1f2p1,f1p1f2m1,f1m1f2p1,f1m1f2m1,kk1p1kk2p1,kk1p1kk2m1,kk1m1kk2p1,kk1m1kk2m1)
                ...
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f1m1f2p1 = F[(g1-1)*W1+(g2+1)];
                    f1m1f2m1 = F[(g1-1)*W1+(g2-1)];
                    kk1p1kk2p1 = KK1[(g1+1)*W1+(g2+1)];
                    kk1p1kk2m1 = KK1[(g1+1)*W1+(g2-1)];
                    kk1m1kk2p1 = KK1[(g1-1)*W1+(g2+1)];
                    kk1m1kk2m1 = KK1[(g1-1)*W1+(g2-1)];
                ...
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f1m1f2p1 = F[(g1-1)*W1+(g2+1)];
                    f1m1f2m1 = F[(g1-1)*W1+(g2-1)];
                    kk1p1kk2p1 = KK2[(g1+1)*W1+(g2+1)];
                    kk1p1kk2m1 = KK2[(g1+1)*W1+(g2-1)];
                    kk1m1kk2p1 = KK2[(g1-1)*W1+(g2+1)];
                    kk1m1kk2m1 = KK2[(g1-1)*W1+(g2-1)];
                ...
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f1m1f2p1 = F[(g1-1)*W1+(g2+1)];
                    f1m1f2m1 = F[(g1-1)*W1+(g2-1)];
                    kk1p1kk2p1 = KK3[(g1+1)*W1+(g2+1)];
                    kk1p1kk2m1 = KK3[(g1+1)*W1+(g2-1)];
                    kk1m1kk2p1 = KK3[(g1-1)*W1+(g2+1)];
                    kk1m1kk2m1 = KK3[(g1-1)*W1+(g2-1)];
            ...
            #pragma omp parallel
            {
                ...
                for (int i1 = 2; i1 < BoxShape[0] - 2; i1 ++)  {
                    for (int i2 = 2; i2 < BoxShape[1] - 2; i2 ++)  {
                        ...
                        KK1[i1*W1+i2] = -kh0m * xx2 * (-f1p2/12.0 + 2/3.0*f1p1 - 2/3.0*f1m1 + f1m2/12.0) + 
                                    k2h1 * POTENTIAL_X(xx1, xx2) * (-f2p2/12.0 + 2/3.0*f2p1 - 2/3.0*f2m1 + f2m2/12.0) -
                                    khbsq2h1 * POTENTIAL_XXX(xx1, xx2) * (-f2p3/8.0+f2p2-13.0*f2p1/8.0+13*f2m1/8.0-f2m2+f2m3/8.0) +
                                    kgamma * (f0 + i2h1 * xx2 * (f2p1 - f2m1) + mkT2h1sq * (f2p1 + f2m1 - 2*f0)) + 
                                    Dqqkh0sq * (f1p1 + f1m1 - 2.0*f0) +
                                    Dpqk4h01 * (f1p1f2p1 - f1p1f2m1 - f1m1f2p1 + f1m1f2m1);
                ...
                for (int i1 = 2; i1 < BoxShape[0] - 2; i1 ++)  {
                    for (int i2 = 2; i2 < BoxShape[1] - 2; i2 ++)  {
                        ...
                        KK2[i1*W1+i2] = -kh0m * xx2 * (-1/12.0*(f1p2+0.5*kk1p2) + 2/3.0*(f1p1+0.5*kk1p1) - 2/3.0*(f1m1+0.5*kk1m1) + 1/12.0*(f1m2+0.5*kk1m2)) + 
                                    k2h1 * POTENTIAL_X(xx1, xx2) * (-1/12.0*(f2p2+0.5*kk2p2) + 2/3.0*(f2p1+0.5*kk2p1) - 2/3.0*(f2m1+0.5*kk2m1) + 1/12.0*(f2m2+0.5*kk2m2)) -
                                    khbsq2h1 * POTENTIAL_XXX(xx1, xx2) * (-(f2p3+0.5*kk2p3)/8.0+(f2p2+0.5*kk2p2)-13.0*(f2p1+0.5*kk2p1)/8.0+13.0*(f2m1+0.5*kk2m1)/8.0-(f2m2+0.5*kk2m2)+(f2m3+0.5*kk2m3)/8.0) +
                                    kgamma * (f0 + 0.5*kk0 + i2h1 * xx2 * (f2p1 + 0.5*kk2p1 - f2m1 - 0.5*kk2m1) + mkT2h1sq * (f2p1 + 0.5*kk2p1 + f2m1 + 0.5*kk2m1 - 2*f0 - kk0)) + 
                                    Dqqkh0sq * (f1p1+0.5*kk1p1 + f1m1+0.5*kk1m1 - 2.0*f0-kk0) +
                                    Dpqk4h01 * ((f1p1f2p1+0.5*kk1p1kk2p1) - (f1p1f2m1+0.5*kk1p1kk2m1) - (f1m1f2p1+0.5*kk1m1kk2p1) + (f1m1f2m1+0.5*kk1m1kk2m1));
                ...
                for (int i1 = 2; i1 < BoxShape[0] - 2; i1 ++)  {
                    for (int i2 = 2; i2 < BoxShape[1] - 2; i2 ++)  {
                        ...
                        KK4[i1*W1+i2] = -kh0m * xx2 * (-1/12.0*(f1p2+kk1p2) + 2/3.0*(f1p1+kk1p1) - 2/3.0*(f1m1+kk1m1) + 1/12.0*(f1m2+kk1m2)) + 
                                    k2h1 * POTENTIAL_X(xx1, xx2) * (-1/12.0*(f2p2+kk2p2) + 2/3.0*(f2p1+kk2p1) - 2/3.0*(f2m1+kk2m1) + 1/12.0*(f2m2+kk2m2)) -
                                    khbsq2h1 * POTENTIAL_XXX(xx1, xx2) * (-(f2p3+kk2p3)/8.0+(f2p2+kk2p2)-13.0*(f2p1+kk2p1)/8.0+13.0*(f2m1+kk2m1)/8.0-(f2m2+kk2m2)+(f2m3+kk2m3)/8.0) +
                                    kgamma * (f0 + kk0 + i2h1 * xx2 * (f2p1 + kk2p1 - f2m1 - kk2m1) + mkT2h1sq * (f2p1 + kk2p1 + f2m1 + kk2m1 - 2*f0 - 2*kk0)) + 
                                    Dqqkh0sq * (f1p1+kk1p1 + f1m1+kk1m1 - 2.0*(f0+kk0)) +
                                    Dpqk4h01 * ((f1p1f2p1+kk1p1kk2p1) - (f1p1f2m1+kk1p1kk2m1) - (f1m1f2p1+kk1m1kk2p1) + (f1m1f2m1+kk1m1kk2m1));
