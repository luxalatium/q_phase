...
void KleinKramers2d::Evolve()
{
    ...
    if ( !isFullGrid )
    {
        ...
        for (int i = 0; i < TB.size(); i++)
        {
            ...
            if ( g1+1 != BoxShape[0]-1 && !TAMask[(g1+1)*W1+g2])
                tmpVec.push_back(GridToIdx(g1+1,g2));
            if ( g1-1 != 0 && !TAMask[(g1-1)*W1+g2])
                tmpVec.push_back(GridToIdx(g1-1,g2));
            if ( g2+1 != BoxShape[1]-1 && !TAMask[g1*W1+(g2+1)])
                tmpVec.push_back(GridToIdx(g1,g2+1));
            if ( g2-1 != 0 && !TAMask[g1*W1+(g2-1)])
                tmpVec.push_back(GridToIdx(g1,g2-1));
        }
    ...
    for (int tt = 0; tt < (int)(TIME / kk); tt ++)
    {
        ...
        while ( TBL.size() != 0 && !isFullGrid && Excount < ExLimit)
        {
            ...
            for ( it = TBL.begin(); it != TBL.end(); it ++ )  {
                ...
                if ( g1-1 != 1 && F[(g1-1)*W1+g2] == 0.0 )  {
                    ExFF.push_back(GridToIdx(g1-1,g2));
                }
                if ( g1+1 != BoxShape[0]-2 && F[(g1+1)*W1+g2] == 0.0 )  {
                    ExFF.push_back(GridToIdx(g1+1,g2));
                }
                if ( g2-1 != 1 && F[g1*W1+(g2-1)] == 0.0 )  {
                    ExFF.push_back(GridToIdx(g1,g2-1));
                }
                if ( g2+1 != BoxShape[1]-2 && F[g1*W1+(g2+1)] == 0.0 )  {
                    ExFF.push_back(GridToIdx(g1,g2+1));
                }
            }
            ...
            if ( isFirstExtrp )  {
                ...
                for (int i = 0; i < ExFF.size(); i++)  {
                    ...
                    if ( g1+1 != BoxShape[0]-2 && !TAMask[(g1+1)*W1+g2] )
                        tmpVec.push_back(GridToIdx(g1+1,g2));
                    if ( g1-1 != 1 && !TAMask[(g1-1)*W1+g2] )
                        tmpVec.push_back(GridToIdx(g1-1,g2));
                    if ( g2+1 != BoxShape[1]-2 && !TAMask[g1*W1+(g2+1)] )
                        tmpVec.push_back(GridToIdx(g1,g2+1));
                    if ( g2-1 != 1 && !TAMask[g1*W1+(g2-1)] )
                        tmpVec.push_back(GridToIdx(g1,g2-1));
                }
