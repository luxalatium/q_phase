...
KleinKramers4d::KleinKramers4d(class QTR *q)
{
    qtr = q;
    err = qtr->error;
    log = qtr->log;
...
KleinKramers4d::~KleinKramers4d()
{     
    return;
}
/* ------------------------------------------------------------------------------- */

void KleinKramers4d::init()
{
    log->log("\n\n[KleinKramers4d] INIT starts ...\n");
    log->log("\n\n[KleinKramers4d] Truncated Grid\n");
    log->log("\n\n[KleinKramers4d] Potential type: %s\n", POTNAME);
    ...
    log->log("[KleinKramers4d] Set skin thickness to %d.\n\n",skin);

    log->log("[KleinKramers4d] INIT done.\n\n");
}
/* ------------------------------------------------------------------------------- */

void KleinKramers4d::Evolve()
{
    ...
    log->log("[KleinKramers4d] Evolve starts ...\n");
    ...
    double k2h1m = kk / (2.0 * H[1] * m);
    double k2h2 = kk / (2.0 * H[2]);
    double k2h3 = kk / (2.0 * H[3]);
    ...
    double kk1p, kk2p, kk3p, kk4p;
    double kk1m, kk2m, kk3m, kk4m;
    ...
    log->log("[KleinKramers4d] nneigh = %d\n", nneigh); 

    // Compute coarse grid shape
    t_1_begin = omp_get_wtime();
    log->log("[KleinKramers4d] Coarse grid size = (");
        ...
        for (int i2 = 0; i2 < BoxShapeCoarse[1]; i2 ++)  {
            for (int i3 = 0; i3 < BoxShapeCoarse[2]; i3 ++)  {
                for (int i4 = 0; i4 < BoxShapeCoarse[3]; i4 ++)  {
                    if ( WAVEFUNCTION(Box[0]+i1*C[0], Box[2]+i2*C[1], Box[4]+i3*C[2], Box[6]+i4*C[3]) >= vTolH )  {
    ...
    if ( x1_min==0 || x2_min==0 || x3_min==0 || x4_min==0 || x1_max==BoxShapeCoarse[0]-1 || x2_max==BoxShapeCoarse[1]-1 || x3_max==BoxShapeCoarse[2]-1 || x4_max==BoxShapeCoarse[3]-1 )  {
        log->log("[KleinKramers4d] Coarse TA range touches boundary");
        exit(1);
    }
    ...
    log->log("[KleinKramers4d] Initital fine grid size = (");
    ...
    if ( !QUIET && TIMING ) log->log("[KleinKramers4d] Elapsed time (make bounding box) = %lf sec\n\n", t_1_elapsed); 

    // Initialize containers
    t_1_begin = omp_get_wtime();

    log->log("[KleinKramers4d] Initializing containers ...\n");
    ...
    for (unsigned long int  i = 0; i < O1; i++)  {
        ...
        if (!TAMask[i])
            log->log("[KleinKramers4d] Memory allocation %s failed\n", "TAMask");
    }
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if ( !QUIET && TIMING ) log->log("[KleinKramers4d] Elapsed time (initializing containers) = %lf sec\n\n", t_1_elapsed); 

    // .........................................................................................

    // Initialize wavefunction
    t_1_begin = omp_get_wtime();
    log->log("[KleinKramers4d] Initializing wavefunction ...\n");  
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (initializing wavefunction) = %lf sec\n\n", t_1_elapsed); 
    ...
    log->log("[KleinKramers4d] Normalization factor = %.16e\n",norm);
    norm = 1.0 / norm;
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (Wavefunction normalization) = %lf sec\n\n", t_1_elapsed); 
    ...
    if ( isCorr )  {
        ...
        log->log("[KleinKramers4d] corr_0 = %.16e\n",corr_0);
        log->log("[KleinKramers4d] Time %lf, Corr = %.16e\n",0.0,1.0);

        t_1_end = omp_get_wtime();
        t_1_elapsed = t_1_end - t_1_begin;
        if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (initial density) = %lf sec\n\n", t_1_elapsed); 
    }
    ...
    log->log("[KleinKramers4d] Initial truncation ...\n");
    ...
    log->log("[KleinKramers4d] TA size = %lu\n", ta_size);
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (truncation) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (TA range) = %lf sec\n\n", t_1_elapsed); 
    ...
    tmpVec.clear();
    tb_size = TB.size();
    log->log("[KleinKramers4d] TB size = %d\n", tb_size);
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (TB) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[KleinKramers4d] Elapsed time (TA expansion) = %lf sec\n\n", t_1_elapsed); 
    ...
    log->log("[KleinKramers4d] TA size = %lu, TB size = %lu\n", ta_size, tb_size);

    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING)  log->log("[KleinKramers4d] Elapsed time (TA,TB range) = %lf sec\n\n", t_1_elapsed);
    ...
    log->log("=======================================================\n\n"); 
    log->log("[KleinKramers4d] Time interation starts ...\n"); 
    log->log("[KleinKramers4d] Number of steps = %d\n\n", (int)(TIME / kk)); 
    log->log("=======================================================\n\n"); 
    ...
    for (unsigned long int tt = 0; tt < (unsigned long int)(TIME / kk); tt ++)
    {
        ...
        while ( TBL.size() != 0 && Excount < ExLimit)
        {
            ...
            if ( isFirstExtrp )  {
                ...
                #pragma omp parallel 
                {
                    ...
                    #pragma omp for private(xx1,xx2,xx3,xx4,f0,f1p,f1m,f2p,f2m,f3p,f3m,f4p,f4m) schedule(runtime)
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        f1p = F[(i1+1)*W1+i2][i3*W2+i4];
                                        f1m = F[(i1-1)*W1+i2][i3*W2+i4];
                                        f2p = F[i1*W1+(i2+1)][i3*W2+i4];
                                        f2m = F[i1*W1+(i2-1)][i3*W2+i4];
                                        f3p = F[i1*W1+i2][(i3+1)*W2+i4];
                                        f3m = F[i1*W1+i2][(i3-1)*W2+i4];
                                        f4p = F[i1*W1+i2][i3*W2+(i4+1)];
                                        f4m = F[i1*W1+i2][i3*W2+(i4-1)];

                                        KK1[i1*W1+i2][i3*W2+i4] = (-k2h0m) * xx3 * (f1p - f1m) +
                                                                (-k2h1m) * xx4 * (f2p - f2m) + 
                                                                k2h2 * POTENTIAL_X1(xx1, xx2) * (f3p - f3m) +
                                                                k2h3 * POTENTIAL_X2(xx1, xx2) * (f4p - f4m) +
                                                                kgamma * (f0 + i2h2 * xx3 * (f3p - f3m) + mkT2h2sq * (f3p + f3m - 2 * f0)) + 
                                                                kgamma * (f0 + i2h3 * xx4 * (f4p - f4m) + mkT2h3sq * (f4p + f4m - 2 * f0));
                    ...
                    // RK4-2
                    #pragma omp for private(xx1,xx2,xx3,xx4,f0,f1p,f1m,f2p,f2m,f3p,f3m,f4p,f4m,kk0,kk1p,kk1m,kk2p,kk2m,kk3p,kk3m,kk4p,kk4m) schedule(runtime)
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        kk1p = KK1[(i1+1)*W1+i2][i3*W2+i4];
                                        kk1m = KK1[(i1-1)*W1+i2][i3*W2+i4];
                                        kk2p = KK1[i1*W1+(i2+1)][i3*W2+i4];
                                        kk2m = KK1[i1*W1+(i2-1)][i3*W2+i4];
                                        kk3p = KK1[i1*W1+i2][(i3+1)*W2+i4];
                                        kk3m = KK1[i1*W1+i2][(i3-1)*W2+i4];
                                        kk4p = KK1[i1*W1+i2][i3*W2+(i4+1)];
                                        kk4m = KK1[i1*W1+i2][i3*W2+(i4-1)];

                                        KK2[i1*W1+i2][i3*W2+i4] =  (-k2h0m) * xx3 * (f1p + 0.5 * kk1p - f1m - 0.5 * kk1m) + 
                                                                (-k2h1m) * xx4 * (f2p + 0.5 * kk2p - f2m - 0.5 * kk2m) + 
                                                                k2h2 * POTENTIAL_X1(xx1, xx2) * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) +
                                                                k2h3 * POTENTIAL_X2(xx1, xx2) * (f4p + 0.5 * kk4p - f4m - 0.5 * kk4m) +
                                                                kgamma * (f0 + 0.5 * kk0 + i2h2 * xx3 * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) + mkT2h2sq * (f3p + 0.5 * kk3p + f3m + 0.5 * kk3m - 2 * f0 - kk0)) + 
                                                                kgamma * (f0 + 0.5 * kk0 + i2h3 * xx4 * (f4p + 0.5 * kk4p - f4m - 0.5 * kk4m) + mkT2h3sq * (f4p + 0.5 * kk4p + f4m + 0.5 * kk4m - 2 * f0 - kk0));
                            ...
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        kk1p = KK2[(i1+1)*W1+i2][i3*W2+i4];
                                        kk1m = KK2[(i1-1)*W1+i2][i3*W2+i4];
                                        kk2p = KK2[i1*W1+(i2+1)][i3*W2+i4];
                                        kk2m = KK2[i1*W1+(i2-1)][i3*W2+i4];
                                        kk3p = KK2[i1*W1+i2][(i3+1)*W2+i4];
                                        kk3m = KK2[i1*W1+i2][(i3-1)*W2+i4];
                                        kk4p = KK2[i1*W1+i2][i3*W2+(i4+1)];
                                        kk4m = KK2[i1*W1+i2][i3*W2+(i4-1)];

                                        KK3[i1*W1+i2][i3*W2+i4] = (-k2h0m) * xx3 * (f1p + 0.5 * kk1p - f1m - 0.5 * kk1m) + 
                                                                (-k2h1m) * xx4 * (f2p + 0.5 * kk2p - f2m - 0.5 * kk2m) + 
                                                                k2h2 * POTENTIAL_X1(xx1, xx2) * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) +
                                                                k2h3 * POTENTIAL_X2(xx1, xx2) * (f4p + 0.5 * kk4p - f4m - 0.5 * kk4m) +
                                                                kgamma * (f0 + 0.5 * kk0 + i2h2 * xx3 * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) + mkT2h2sq * (f3p + 0.5 * kk3p + f3m + 0.5 * kk3m - 2 * f0 - kk0)) + 
                            ...
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        kk1p = KK3[(i1+1)*W1+i2][i3*W2+i4];
                                        kk1m = KK3[(i1-1)*W1+i2][i3*W2+i4];
                                        kk2p = KK3[i1*W1+(i2+1)][i3*W2+i4];
                                        kk2m = KK3[i1*W1+(i2-1)][i3*W2+i4];
                                        kk3p = KK3[i1*W1+i2][(i3+1)*W2+i4];
                                        kk3m = KK3[i1*W1+i2][(i3-1)*W2+i4];
                                        kk4p = KK3[i1*W1+i2][i3*W2+(i4+1)];
                                        kk4m = KK3[i1*W1+i2][i3*W2+(i4-1)];

                                        KK4[i1*W1+i2][i3*W2+i4] = (-k2h0m) * xx3 * (f1p + kk1p - f1m - kk1m) + 
                                                                (-k2h1m) * xx4 * (f2p + kk2p - f2m - kk2m) + 
                                                                k2h2 * POTENTIAL_X1(xx1, xx2) * (f3p + kk3p - f3m - kk3m) +
                                                                k2h3 * POTENTIAL_X2(xx1, xx2) * (f4p + kk4p - f4m - kk4m) +
                                                                kgamma * (f0 + kk0 + i2h2 * xx3 * (f3p + kk3p - f3m - kk3m) + mkT2h2sq * (f3p + kk3p + f3m + kk3m - 2 * f0 - 2 * kk0)) + 
                                                                kgamma * (f0 + kk0 + i2h3 * xx4 * (f4p + kk4p - f4m - kk4m) + mkT2h3sq * (f4p + kk4p + f4m + kk4m - 2 * f0 - 2 * kk0));
            ...
            else  
            {
                ...
                // RK4-1
                #pragma omp parallel for private(g1,g2,g3,g4,xx1,xx2,xx3,xx4,f0,f1p,f1m,f2p,f2m,f3p,f3m,f4p,f4m) schedule(runtime)
                for (unsigned long int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f1p = F[(g1+1)*W1+g2][g3*W2+g4];
                    f1m = F[(g1-1)*W1+g2][g3*W2+g4];
                    f2p = F[g1*W1+(g2+1)][g3*W2+g4];
                    f2m = F[g1*W1+(g2-1)][g3*W2+g4];
                    f3p = F[g1*W1+g2][(g3+1)*W2+g4];
                    f3m = F[g1*W1+g2][(g3-1)*W2+g4];
                    f4p = F[g1*W1+g2][g3*W2+(g4+1)];
                    f4m = F[g1*W1+g2][g3*W2+(g4-1)];

                    KK1[g1*W1+g2][g3*W2+g4] = (-k2h0m) * xx3 * (f1p - f1m) +
                                            (-k2h1m) * xx4 * (f2p - f2m) +
                                            k2h2 * POTENTIAL_X1(xx1,xx2) * (f3p - f3m) +
                                            k2h3 * POTENTIAL_X2(xx1,xx2) * (f4p - f4m) +
                                            kgamma * (f0 + i2h2 * xx3 * (f3p - f3m) + mkT2h2sq * (f3p + f3m - 2 * f0)) + 
                ...
                // RK4-2
                #pragma omp parallel for private(g1,g2,g3,g4,xx1,xx2,xx3,xx4,f0,f1p,f1m,f2p,f2m,f3p,f3m,f4p,f4m,kk0,kk1p,kk1m,kk2p,kk2m,kk3p,kk3m,kk4p,kk4m) schedule(runtime)
                for (unsigned long int i = 0; i < ExBD.size(); i++)  {
                    ...
                    kk1p = KK1[(g1+1)*W1+g2][g3*W2+g4];
                    kk1m = KK1[(g1-1)*W1+g2][g3*W2+g4];
                    kk2p = KK1[g1*W1+(g2+1)][g3*W2+g4];
                    kk2m = KK1[g1*W1+(g2-1)][g3*W2+g4];
                    kk3p = KK1[g1*W1+g2][(g3+1)*W2+g4];
                    kk3m = KK1[g1*W1+g2][(g3-1)*W2+g4];
                    kk4p = KK1[g1*W1+g2][g3*W2+(g4+1)];
                    kk4m = KK1[g1*W1+g2][g3*W2+(g4-1)];

                    KK2[g1*W1+g2][g3*W2+g4] = (-k2h0m) * xx3 * (f1p + 0.5 * kk1p - f1m - 0.5 * kk1m) + 
                                            (-k2h1m) * xx4 * (f2p + 0.5 * kk2p - f2m - 0.5 * kk2m) + 
                                            k2h2 * POTENTIAL_X1(xx1,xx2) * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) +
                                            k2h3 * POTENTIAL_X2(xx1,xx2) * (f4p + 0.5 * kk4p - f4m - 0.5 * kk4m) +
                                            kgamma * (f0 + 0.5 * kk0 + i2h2 * xx3 * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) + mkT2h2sq * (f3p + 0.5 * kk3p + f3m + 0.5 * kk3m - 2 * f0 - kk0)) + 
                ...
                for (unsigned long int i = 0; i < ExBD.size(); i++)  {
                    ...
                    kk1p = KK2[(g1+1)*W1+g2][g3*W2+g4];
                    kk1m = KK2[(g1-1)*W1+g2][g3*W2+g4];
                    kk2p = KK2[g1*W1+(g2+1)][g3*W2+g4];
                    kk2m = KK2[g1*W1+(g2-1)][g3*W2+g4];
                    kk3p = KK2[g1*W1+g2][(g3+1)*W2+g4];
                    kk3m = KK2[g1*W1+g2][(g3-1)*W2+g4];
                    kk4p = KK2[g1*W1+g2][g3*W2+(g4+1)];
                    kk4m = KK2[g1*W1+g2][g3*W2+(g4-1)];

                    KK3[g1*W1+g2][g3*W2+g4] = (-k2h0m) * xx3 * (f1p + 0.5 * kk1p - f1m - 0.5 * kk1m) + 
                                            (-k2h1m) * xx4 * (f2p + 0.5 * kk2p - f2m - 0.5 * kk2m) + 
                                            k2h2 * POTENTIAL_X1(xx1,xx2) * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) +
                                            k2h3 * POTENTIAL_X2(xx1,xx2) * (f4p + 0.5 * kk4p - f4m - 0.5 * kk4m) +
                                            kgamma * (f0 + 0.5 * kk0 + i2h2 * xx3 * (f3p + 0.5 * kk3p - f3m - 0.5 * kk3m) + mkT2h2sq * (f3p + 0.5 * kk3p + f3m + 0.5 * kk3m - 2 * f0 - kk0)) + 
                ...
                for (unsigned long int i = 0; i < ExBD.size(); i++)  {
                    ...
                    kk1p = KK3[(g1+1)*W1+g2][g3*W2+g4];
                    kk1m = KK3[(g1-1)*W1+g2][g3*W2+g4];
                    kk2p = KK3[g1*W1+(g2+1)][g3*W2+g4];
                    kk2m = KK3[g1*W1+(g2-1)][g3*W2+g4];
                    kk3p = KK3[g1*W1+g2][(g3+1)*W2+g4];
                    kk3m = KK3[g1*W1+g2][(g3-1)*W2+g4];
                    kk4p = KK3[g1*W1+g2][g3*W2+(g4+1)];
                    kk4m = KK3[g1*W1+g2][g3*W2+(g4-1)];

                    KK4[g1*W1+g2][g3*W2+g4] = (-k2h0m) * xx3 * (f1p + kk1p - f1m - kk1m) + 
                                            (-k2h1m) * xx4 * (f2p + kk2p - f2m - kk2m) + 
                                            k2h2 * POTENTIAL_X1(xx1,xx2) * (f3p + kk3p -  f3m - kk3m) +
                                            k2h3 * POTENTIAL_X2(xx1,xx2) * (f4p + kk4p -  f4m - kk4m) +
                                            kgamma * (f0 + kk0 + i2h2 * xx3 * (f3p + kk3p - f3m - kk3m) + mkT2h2sq * (f3p + kk3p + f3m + kk3m - 2 * f0 - 2 * kk0)) + 
        ...
        if ( (tt + 1) % PERIOD == 0 )
        {
            ...
            if (isTrans)  {
                ...
                log->log("[KleinKramers4d] trans_x0 = %lf\n", idx_x0 * H[0] + BoxDynamic[0]);
                log->log("[KleinKramers4d] Time %lf, Trans = %.16e\n", ( tt + 1 ) * kk, pftrans);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
        ...
        if ( isTouchedBoundary )  {
            ...
            if ( (BoxDynamic[0] == BoxBoundary[0]) && x1_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x1_min to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[0] = %lf, dist = %d\n", BoxBoundary[0], x1_min);
                ...
                x1_min = 3;
                log->log("[KleinKramers4d] New x1_min = %d\n", x1_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[1] == BoxBoundary[1]) && (int)(std::round(BoxBoundary[1] - (BoxDynamic[0] + x1_max * H[0]))/H[0]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x1_max to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[1] = %lf, dist = %d\n",BoxBoundary[1], (int)(std::round(BoxBoundary[1] - (BoxDynamic[0] + x1_max * H[0]))/H[0]));
                ...
                log->log("[KleinKramers4d] New x1_max = %d\n", x1_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[2] == BoxBoundary[2]) && x2_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x2_min to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[2] = %lf, dist = %d\n", BoxBoundary[2], x2_min);
                ...
                x2_min = 3;
                log->log("[KleinKramers4d] New x2_min = %d\n", x2_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[3] == BoxBoundary[3]) && (int)(std::round(BoxBoundary[3] - (BoxDynamic[2] + x2_max * H[1]))/H[1]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x2_max to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[3] = %lf, dist = %d\n",BoxBoundary[3], (int)(std::round(BoxBoundary[3] - (BoxDynamic[2] + x2_max * H[1]))/H[1]));
                ...
                log->log("[KleinKramers4d] New x2_max = %d\n", x2_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[4] == BoxBoundary[4]) && x3_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x3_min to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[4] = %lf, dist = %d\n", BoxBoundary[4], x3_min);
                ...
                x3_min = 3;
                log->log("[KleinKramers4d] New x3_min = %d\n", x3_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[5] == BoxBoundary[5]) && (int)(std::round(BoxBoundary[5] - (BoxDynamic[4] + x3_max * H[2]))/H[2]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x3_max to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[5] = %lf, dist = %d\n", BoxBoundary[5], (int)(std::round(BoxBoundary[5] - (BoxDynamic[4] + x3_max * H[2]))/H[2]));
                ...
                log->log("[KleinKramers4d] New x3_max = %d\n", x3_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[6] == BoxBoundary[6]) && x4_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x4_min to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[6] = %lf, dist = %d\n", BoxBoundary[6], x4_min);
                ...
                x4_min = 3;
                log->log("[KleinKramers4d] New x4_min = %d\n", x4_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[7] == BoxBoundary[7]) && (int)(std::round(BoxBoundary[7] - (BoxDynamic[6] + x4_max * H[3]))/H[3]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[KleinKramers4d] x4_max to boundary is less than 3\n");
                log->log("[KleinKramers4d] BoxBoundary[7] = %lf, dist = %d\n", BoxBoundary[7], (int)(std::round(BoxBoundary[7] - (BoxDynamic[6] + x4_max * H[3]))/H[3]));
                ...
                log->log("[KleinKramers4d] New x4_max = %d\n", x4_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
        ...
        {   
            ...
            if ( !QUIET )  {
                ...
                log->log("[KleinKramers4d] Step: %lu, Elapsed time: %lf sec\n", tt + 1, t_0_elapsed);
                ta_size = 0;
                ...
                tb_size = TB.size();
                log->log("[KleinKramers4d] TA size = %lu, TB size = %lu\n", ta_size, tb_size);
                log->log("[KleinKramers4d] BoxPosition: [%.2f,%.2f][%.2f,%.2f][%.2f,%.2f][%.2f,%.2f]\n",BoxDynamic[0],BoxDynamic[1],BoxDynamic[2],BoxDynamic[3],BoxDynamic[4],BoxDynamic[5],BoxDynamic[6],BoxDynamic[7]);
                log->log("[KleinKramers4d] BoxShape: [%d,%d,%d,%d]\n",BoxShapeFine[0],BoxShapeFine[1],BoxShapeFine[2],BoxShapeFine[3]);
                log->log("[KleinKramers4d] TA Range [%d, %d][%d, %d][%d, %d][%d, %d]\n", x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max);
                log->log("[KleinKramers4d] TA / total grids = %lf\n", ( ta_size * 1.0 ) / GRIDS_TOT);
                log->log("[KleinKramers4d] ExCount = %d ExLimit = %d\n", Excount, ExLimit);
                log->log("\n........................................................\n\n");
            }
        }         
    } // Time iteration 
    ...
    log->log("[KleinKramers4d] Evolve done.\n");
}
/* =============================================================================== */

/* KK4DPOT_DW1 */

inline double KleinKramers4d::Wavefunction_DW1(double x1, double x2, double x3, double x4)
{
...
inline double KleinKramers4d::Potential_DW1(double x1, double x2)
{
    return 0.025 * (x1 * x1 - 1.0) * (x1 * x1 - 1.0) + 0.5 * k0 * (1.0 - sigma * exp(- lambda * x1 * x1)) * x2 * x2;    
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Vx1_DW1(double x1, double x2)
{
    return 0.1 * x1 * (x1 * x1 - 1.0) + k0 * sigma * lambda * x1 * x2 * x2 * exp(-lambda * x1 * x1);
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Vx2_DW1(double x1, double x2)
{
...
inline double KleinKramers4d::Wavefunction_DW2(double x1, double x2, double x3, double x4)
{
    return PIHBSQ_INV * exp(-2.0 * (A[0]*pow(x1-Wave0[0],2)+A[1]*pow(x2-Wave0[1],2))) * exp( -0.5 * HBSQ_INV * (pow(x3-Wave0[2],2)/A[2]+pow(x4-Wave0[3],2)/A[3]) );
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Potential_DW2(double x1, double x2)
{
    return 0.007 * (x1 * x1 * x1 * x1) - 0.01 * x1 * x1 + 0.5 * k0 * (1.0 - sigma * exp(- lambda * x1 * x1)) * x2 * x2;    
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Vx1_DW2(double x1, double x2)
{
    return 0.028 * (x1 * x1 * x1) - 0.02 * x1 + k0 * sigma * lambda * x1 * x2 * x2 * exp(-lambda * x1 * x1);
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Vx2_DW2(double x1, double x2)
{
    return k0 * x2 * (1.0 - sigma * exp(-lambda * x1 * x1));
}
/* =============================================================================== */
...
inline double KleinKramers4d::Wavefunction_DW3(double x1, double x2, double x3, double x4)
{
    return PIHBSQ_INV * exp(-2.0 * (A[0]*pow(x1-Wave0[0],2)+A[1]*pow(x2-Wave0[1],2))) * exp( -0.5 * HBSQ_INV * (pow(x3-Wave0[2],2)/A[2]+pow(x4-Wave0[3],2)/A[3]) );
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Potential_DW3(double x1, double x2)
{
    return 3.0 * x1 * x1 * x1 * x1 - 8.0 * x1 * x1 + x2 * x2 - 0.1 * x1 * x2;    
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Vx1_DW3(double x1, double x2)
{
    return 12.0 * x1 * x1 * x1 - 16.0 * x1 - 0.1 * x2;
}
/* ------------------------------------------------------------------------------- */

inline double KleinKramers4d::Vx2_DW3(double x1, double x2)
{
    return 2.0 * x2 - 0.1 * x1;
}
/* ------------------------------------------------------------------------------- */

VectorXi KleinKramers4d::IdxToGrid(unsigned long int idx)
{
    int x4 = (int)( idx % M3 );
...
inline unsigned long int KleinKramers4d::GridToIdx(int x1, int x2, int x3, int x4)
{
