...
{
    log->log("\n\n[Diosi4d] INIT starts ...\n");
    log->log("\n\n[Diosi4d] Using Truncated Grid\n");
    log->log("\n\n[Diosi4d] Potential type: %s\n", POTNAME);
    ...
    log->log("[Diosi4d] Set skin thickness to %d.\n\n",skin);

    log->log("[Diosi4d] INIT done.\n\n");
}
/* ------------------------------------------------------------------------------- */
...
void Diosi4d::Evolve()
{
    ...
    double Dqq = gamma * hb * hb / (12.0 * m * kb * temp);
    double Dpq = gamma * hb * hb * omega / (6.0 * PI * kb * temp);
    double Dqqkh0sq = Dqq * kk / (H[0] * H[0]);
    double Dqqkh1sq = Dqq * kk / (H[1] * H[1]);
    double Dpqk4h02 = Dpq * kk / (4.0 * H[0] * H[2]);
    double Dpqk4h13 = Dpq * kk / (4.0 * H[1] * H[3]);
    ...
    double f1p1f3p1, f1p1f3m1;
    double f1m1f3p1, f1m1f3m1;
    double f2p1f4p1, f2p1f4m1;
    double f2m1f4p1, f2m1f4m1;
    double kk1p1kk3p1, kk1p1kk3m1;
    double kk1m1kk3p1, kk1m1kk3m1;
    double kk2p1kk4p1, kk2p1kk4m1;
    double kk2m1kk4p1, kk2m1kk4m1;
    ...
    log->log("[Diosi4d] nneigh = %d\n", nneigh); 

    // Compute coarse grid shape
    t_1_begin = omp_get_wtime();
    log->log("[Diosi4d] Coarse grid size = (");
    ...
    if ( x1_min==0 || x2_min==0 || x3_min==0 || x4_min==0 || x1_max==BoxShapeCoarse[0]-1 || x2_max==BoxShapeCoarse[1]-1 || x3_max==BoxShapeCoarse[2]-1 || x4_max==BoxShapeCoarse[3]-1 )  {
        log->log("[Diosi4d] Coarse TA range touches boundary");
        exit(1);
    }
    ...
    log->log("[Diosi4d] Initital fine grid size = (");
    ...
    if ( !QUIET && TIMING ) log->log("[Diosi4d] Elapsed time (make bounding box) = %lf sec\n\n", t_1_elapsed); 

    // Initialize containers
    t_0_begin = omp_get_wtime();

    log->log("[Diosi4d] Initializing containers ...\n");
    ...
    for (unsigned long int  i = 0; i < O1; i++)  {
        ...
        if (!TAMask[i])
            log->log("[Diosi4d] Memory allocation %s failed\n", "TAMask");
    }
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if ( !QUIET && TIMING ) log->log("[Diosi4d] Elapsed time (initializing containers) = %lf sec\n\n", t_1_elapsed); 

    // .........................................................................................

    // Initialize wavefunction
    t_1_begin = omp_get_wtime();
    log->log("[Diosi4d] Initializing wavefunction ...\n");  
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (initializing wavefunction) = %lf sec\n\n", t_1_elapsed); 
    ...
    log->log("[Diosi4d] Normalization factor = %.16e\n",norm);
    norm = 1.0 / norm;
    ...
    if ( isCorr )  {
        ...
        log->log("[Diosi4d] corr_0 = %.16e\n",corr_0);
        log->log("[Diosi4d] Time %lf, Corr = %.16e\n",0.0,1.0);

        t_1_end = omp_get_wtime();
        t_1_elapsed = t_1_end - t_1_begin;
        if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (initial density) = %lf sec\n\n", t_1_elapsed); 
    }
    ...
    log->log("[Diosi4d] Initial truncation ...\n");
    ...
    log->log("[Diosi4d] TA size = %lu\n", ta_size);
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (truncation) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (TA range) = %lf sec\n\n", t_1_elapsed); 
    ...
    tmpVec.clear();
    tb_size = TB.size();
    log->log("[Diosi4d] TB size = %d\n", tb_size);
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (TB) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (TA expansion) = %lf sec\n\n", t_1_elapsed); 
    ...
    log->log("[Diosi4d] TA size = %lu, TB size = %lu\n", ta_size, tb_size);

    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING)  log->log("[Diosi4d] Elapsed time (TA,TB range) = %lf sec\n\n", t_1_elapsed);
    // .........................................................................................

    // Time iteration 

    log->log("=======================================================\n\n"); 
    log->log("[Diosi4d] Time interation starts ...\n"); 
    log->log("[Diosi4d] Number of steps = %d\n\n", (int)(TIME / kk)); 
    log->log("=======================================================\n\n"); 
    ...
    for (unsigned long int tt = 0; tt < (unsigned long int)(TIME / kk); tt ++)
    {
        ...
        while ( TBL.size() != 0 && Excount < ExLimit )
        {
            ...
            if ( isFirstExtrp )  {
                ...
                #pragma omp parallel 
                {
                    ...
                    #pragma omp for private(xx1,xx2,xx3,xx4,f0,f1p1,f1m1,f2p1,f2m1,f3p1,f3m1,f4p1,f4m1,f1p2,f1m2,f2p2,f2m2,f3p2,f3m2,f4p2,f4m2,f1p1f3p1,f1p1f3m1,f1m1f3p1,f1m1f3m1,f2p1f4p1,f2p1f4m1,f2m1f4p1,f2m1f4m1) schedule(runtime)
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        f1p1f3p1 = F[(i1+1)*W1+i2][(i3+1)*W2+i4];
                                        f1p1f3m1 = F[(i1+1)*W1+i2][(i3-1)*W2+i4];
                                        f1m1f3p1 = F[(i1-1)*W1+i2][(i3+1)*W2+i4];
                                        f1m1f3m1 = F[(i1-1)*W1+i2][(i3-1)*W2+i4];
                                        f2p1f4p1 = F[i1*W1+(i2+1)][i3*W2+(i4+1)];
                                        f2p1f4m1 = F[i1*W1+(i2+1)][i3*W2+(i4-1)];
                                        f2m1f4p1 = F[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        f2m1f4m1 = F[i1*W1+(i2-1)][i3*W2+(i4-1)];
                                        ...
                                        KK1[i1*W1+i2][i3*W2+i4] = (-kh0m) * xx3 * (-f1p2/12.0 + 2/3.0*f1p1 - 2/3.0*f1m1 + f1m2/12.0) +
                                                                ...
                                                                Dqqkh1sq * (f2p1 + f2m1 - 2.0*f0) +
                                                                Dpqk4h02 * (f1p1f3p1 - f1p1f3m1 - f1m1f3p1 + f1m1f3m1) +
                                                                Dpqk4h13 * (f2p1f4p1 - f2p1f4m1 - f2m1f4p1 + f2m1f4m1);
                    ...
                    // RK4-2
                    #pragma omp for private(xx1,xx2,xx3,xx4,f0,f1p1,f1m1,f2p1,f2m1,f3p1,f3m1,f4p1,f4m1,f1p2,f1m2,f2p2,f2m2,f3p2,f3m2,f4p2,f4m2,kk0,kk1p1,kk1m1,kk2p1,kk2m1,kk3p1,kk3m1,kk4p1,kk4m1,kk1p2,kk1m2,kk2p2,kk2m2,kk3p2,kk3m2,kk4p2,kk4m2,f1p1f3p1,f1p1f3m1,f1m1f3p1,f1m1f3m1,f2p1f4p1,f2p1f4m1,f2m1f4p1,f2m1f4m1,kk1p1kk3p1,kk1p1kk3m1,kk1m1kk3p1,kk1m1kk3m1,kk2p1kk4p1,kk2p1kk4m1,kk2m1kk4p1,kk2m1kk4m1) schedule(runtime)
                    for (int i1 = x1_min; i1 <= x1_max; i1 ++)  {
                        for (int i2 = x2_min; i2 <= x2_max; i2 ++)  {
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        f2m1f4p1 = F[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        f2m1f4m1 = F[i1*W1+(i2-1)][i3*W2+(i4-1)];
                                        kk1p1kk3p1 = KK1[(i1+1)*W1+i2][(i3+1)*W2+i4];
                                        kk1p1kk3m1 = KK1[(i1+1)*W1+i2][(i3-1)*W2+i4];
                                        kk1m1kk3p1 = KK1[(i1-1)*W1+i2][(i3+1)*W2+i4];
                                        kk1m1kk3m1 = KK1[(i1-1)*W1+i2][(i3-1)*W2+i4];
                                        kk2p1kk4p1 = KK1[i1*W1+(i2+1)][i3*W2+(i4+1)];
                                        kk2p1kk4m1 = KK1[i1*W1+(i2+1)][i3*W2+(i4-1)];
                                        kk2m1kk4p1 = KK1[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        kk2m1kk4m1 = KK1[i1*W1+(i2-1)][i3*W2+(i4-1)];
                                        ...
                                        KK2[i1*W1+i2][i3*W2+i4] = (-kh0m) * xx3 * (-1/12.0*(f1p2+0.5*kk1p2) + 2/3.0*(f1p1+0.5*kk1p1) - 2/3.0*(f1m1+0.5*kk1m1) + 1/12.0*(f1m2+0.5*kk1m2)) + 
                                                                  ...
                                                                  Dqqkh1sq * (f2p1+0.5*kk2p1 + f2m1+0.5*kk2m1 - 2.0*f0-kk0) +
                                                                  Dpqk4h02 * ((f1p1f3p1+0.5*kk1p1kk3p1) - (f1p1f3m1+0.5*kk1p1kk3m1) - (f1m1f3p1+0.5*kk1m1kk3p1) + (f1m1f3m1+0.5*kk1m1kk3m1)) +
                                                                  Dpqk4h13 * ((f2p1f4p1+0.5*kk2p1kk4p1) - (f2p1f4m1+0.5*kk2p1kk4m1) - (f2m1f4p1+0.5*kk2m1kk4p1) + (f2m1f4m1+0.5*kk2m1kk4m1));
                            ...
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        f2m1f4p1 = F[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        f2m1f4m1 = F[i1*W1+(i2-1)][i3*W2+(i4-1)];
                                        kk1p1kk3p1 = KK2[(i1+1)*W1+i2][(i3+1)*W2+i4];
                                        kk1p1kk3m1 = KK2[(i1+1)*W1+i2][(i3-1)*W2+i4];
                                        kk1m1kk3p1 = KK2[(i1-1)*W1+i2][(i3+1)*W2+i4];
                                        kk1m1kk3m1 = KK2[(i1-1)*W1+i2][(i3-1)*W2+i4];
                                        kk2p1kk4p1 = KK2[i1*W1+(i2+1)][i3*W2+(i4+1)];
                                        kk2p1kk4m1 = KK2[i1*W1+(i2+1)][i3*W2+(i4-1)];
                                        kk2m1kk4p1 = KK2[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        kk2m1kk4m1 = KK2[i1*W1+(i2-1)][i3*W2+(i4-1)];
                            ...
                            for (int i3 = x3_min; i3 <= x3_max; i3 ++)  {
                                for (int i4 = x4_min; i4 <= x4_max; i4 ++)  {
                                    if ( TAMask[i1*W1+i2][i3*W2+i4] )  {
                                        ...
                                        f2m1f4p1 = F[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        f2m1f4m1 = F[i1*W1+(i2-1)][i3*W2+(i4-1)];
                                        kk1p1kk3p1 = KK3[(i1+1)*W1+i2][(i3+1)*W2+i4];
                                        kk1p1kk3m1 = KK3[(i1+1)*W1+i2][(i3-1)*W2+i4];
                                        kk1m1kk3p1 = KK3[(i1-1)*W1+i2][(i3+1)*W2+i4];
                                        kk1m1kk3m1 = KK3[(i1-1)*W1+i2][(i3-1)*W2+i4];
                                        kk2p1kk4p1 = KK3[i1*W1+(i2+1)][i3*W2+(i4+1)];
                                        kk2p1kk4m1 = KK3[i1*W1+(i2+1)][i3*W2+(i4-1)];
                                        kk2m1kk4p1 = KK3[i1*W1+(i2-1)][i3*W2+(i4+1)];
                                        kk2m1kk4m1 = KK3[i1*W1+(i2-1)][i3*W2+(i4-1)];
                                        ...
                                        KK4[i1*W1+i2][i3*W2+i4] = (-kh0m) * xx3 * (-1/12.0*(f1p2+kk1p2) + 2/3.0*(f1p1+kk1p1) - 2/3.0*(f1m1+kk1m1) + 1/12.0*(f1m2+kk1m2)) +
                                                      ...
                                                      Dqqkh1sq * (f2p1+kk2p1 + f2m1+kk2m1 - 2.0*(f0+kk0)) +
                                                      Dpqk4h02 * ((f1p1f3p1+kk1p1kk3p1) - (f1p1f3m1+kk1p1kk3m1) - (f1m1f3p1+kk1m1kk3p1) + (f1m1f3m1+kk1m1kk3m1)) +
                                                      Dpqk4h13 * ((f2p1f4p1+kk2p1kk4p1) - (f2p1f4m1+kk2p1kk4m1) - (f2m1f4p1+kk2m1kk4p1) + (f2m1f4m1+kk2m1kk4m1));
            ...
            else  
            {
                ...
                // RK4-1
                #pragma omp for private(g1,g2,g3,g4,xx1,xx2,xx3,xx4,f0,f1p1,f1m1,f2p1,f2m1,f3p1,f3m1,f4p1,f4m1,f1p2,f1m2,f2p2,f2m2,f3p2,f3m2,f4p2,f4m2,f1p1f3p1,f1p1f3m1,f1m1f3p1,f1m1f3m1,f2p1f4p1,f2p1f4m1,f2m1f4p1,f2m1f4m1) schedule(runtime)
                for (unsigned long int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f1p1f3p1 = F[(g1+1)*W1+g2][(g3+1)*W2+g4];
                    f1p1f3m1 = F[(g1+1)*W1+g2][(g3-1)*W2+g4];
                    f1m1f3p1 = F[(g1-1)*W1+g2][(g3+1)*W2+g4];
                    f1m1f3m1 = F[(g1-1)*W1+g2][(g3-1)*W2+g4];
                    f2p1f4p1 = F[g1*W1+(g2+1)][g3*W2+(g4+1)];
                    f2p1f4m1 = F[g1*W1+(g2+1)][g3*W2+(g4-1)];
                    f2m1f4p1 = F[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    f2m1f4m1 = F[g1*W1+(g2-1)][g3*W2+(g4-1)];
                ...
                // RK4-2
                #pragma omp parallel for private(g1,g2,g3,g4,xx1,xx2,xx3,xx4,f0,f1p1,f1m1,f2p1,f2m1,f3p1,f3m1,f4p1,f4m1,f1p2,f1m2,f2p2,f2m2,f3p2,f3m2,f4p2,f4m2,kk0,kk1p1,kk1m1,kk2p1,kk2m1,kk3p1,kk3m1,kk4p1,kk4m1,kk1p2,kk1m2,kk2p2,kk2m2,kk3p2,kk3m2,kk4p2,kk4m2,f1p1f3p1,f1p1f3m1,f1m1f3p1,f1m1f3m1,f2p1f4p1,f2p1f4m1,f2m1f4p1,f2m1f4m1,kk1p1kk3p1,kk1p1kk3m1,kk1m1kk3p1,kk1m1kk3m1,kk2p1kk4p1,kk2p1kk4m1,kk2m1kk4p1,kk2m1kk4m1) schedule(runtime)
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f2m1f4p1 = F[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    f2m1f4m1 = F[g1*W1+(g2-1)][g3*W2+(g4-1)];
                    kk1p1kk3p1 = KK1[(g1+1)*W1+g2][(g3+1)*W2+g4];
                    kk1p1kk3m1 = KK1[(g1+1)*W1+g2][(g3-1)*W2+g4];
                    kk1m1kk3p1 = KK1[(g1-1)*W1+g2][(g3+1)*W2+g4];
                    kk1m1kk3m1 = KK1[(g1-1)*W1+g2][(g3-1)*W2+g4];
                    kk2p1kk4p1 = KK1[g1*W1+(g2+1)][g3*W2+(g4+1)];
                    kk2p1kk4m1 = KK1[g1*W1+(g2+1)][g3*W2+(g4-1)];
                    kk2m1kk4p1 = KK1[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    kk2m1kk4m1 = KK1[g1*W1+(g2-1)][g3*W2+(g4-1)];
                ...
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f2m1f4p1 = F[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    f2m1f4m1 = F[g1*W1+(g2-1)][g3*W2+(g4-1)];
                    kk1p1kk3p1 = KK2[(g1+1)*W1+g2][(g3+1)*W2+g4];
                    kk1p1kk3m1 = KK2[(g1+1)*W1+g2][(g3-1)*W2+g4];
                    kk1m1kk3p1 = KK2[(g1-1)*W1+g2][(g3+1)*W2+g4];
                    kk1m1kk3m1 = KK2[(g1-1)*W1+g2][(g3-1)*W2+g4];
                    kk2p1kk4p1 = KK2[g1*W1+(g2+1)][g3*W2+(g4+1)];
                    kk2p1kk4m1 = KK2[g1*W1+(g2+1)][g3*W2+(g4-1)];
                    kk2m1kk4p1 = KK2[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    kk2m1kk4m1 = KK2[g1*W1+(g2-1)][g3*W2+(g4-1)];
                ...
                // RK4-4
                #pragma omp for private(g1,g2,g3,g4,xx1,xx2,xx3,xx4,f0,f1p1,f1m1,f2p1,f2m1,f3p1,f3m1,f4p1,f4m1,f1p2,f1m2,f2p2,f2m2,f3p2,f3m2,f4p2,f4m2,kk0,kk1p1,kk1m1,kk2p1,kk2m1,kk3p1,kk3m1,kk4p1,kk4m1,kk1p2,kk1m2,kk2p2,kk2m2,kk3p2,kk3m2,kk4p2,kk4m2,f1p1f3p1,f1p1f3m1,f1m1f3p1,f1m1f3m1,f2p1f4p1,f2p1f4m1,f2m1f4p1,f2m1f4m1,kk1p1kk3p1,kk1p1kk3m1,kk1m1kk3p1,kk1m1kk3m1,kk2p1kk4p1,kk2p1kk4m1,kk2m1kk4p1,kk2m1kk4m1) schedule(runtime)
                for (int i = 0; i < ExBD.size(); i++)  {
                    ...
                    f2m1f4p1 = F[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    f2m1f4m1 = F[g1*W1+(g2-1)][g3*W2+(g4-1)];
                    kk1p1kk3p1 = KK3[(g1+1)*W1+g2][(g3+1)*W2+g4];
                    kk1p1kk3m1 = KK3[(g1+1)*W1+g2][(g3-1)*W2+g4];
                    kk1m1kk3p1 = KK3[(g1-1)*W1+g2][(g3+1)*W2+g4];
                    kk1m1kk3m1 = KK3[(g1-1)*W1+g2][(g3-1)*W2+g4];
                    kk2p1kk4p1 = KK3[g1*W1+(g2+1)][g3*W2+(g4+1)];
                    kk2p1kk4m1 = KK3[g1*W1+(g2+1)][g3*W2+(g4-1)];
                    kk2m1kk4p1 = KK3[g1*W1+(g2-1)][g3*W2+(g4+1)];
                    kk2m1kk4m1 = KK3[g1*W1+(g2-1)][g3*W2+(g4-1)];
        ...
        if ( (tt + 1) % PERIOD == 0 )
        {
            ...
            if (isTrans)  {
                ...
                log->log("[Diosi4d] trans_x0 = %lf\n", idx_x0 * H[0] + BoxDynamic[0]);
                log->log("[Diosi4d] Time %lf, Trans = %.16e\n", ( tt + 1 ) * kk, pftrans);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
        ...
        if ( isTouchedBoundary )  {
            ...
            if ( (BoxDynamic[0] == BoxBoundary[0]) && x1_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x1_min to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[0] = %lf, dist = %d\n", BoxBoundary[0], x1_min);
            ...
            if ( (BoxDynamic[1] == BoxBoundary[1]) && (int)(std::round(BoxBoundary[1] - (BoxDynamic[0] + x1_max * H[0]))/H[0]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x1_max to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[1] = %lf, dist = %d\n",BoxBoundary[1], (int)(std::round(BoxBoundary[1] - (BoxDynamic[0] + x1_max * H[0]))/H[0]));
            ...
            if ( (BoxDynamic[2] == BoxBoundary[2]) && x2_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x2_min to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[2] = %lf, dist = %d\n", BoxBoundary[2], x2_min);
            ...
            if ( (BoxDynamic[3] == BoxBoundary[3]) && (int)(std::round(BoxBoundary[3] - (BoxDynamic[2] + x2_max * H[1]))/H[1]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x2_max to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[3] = %lf, dist = %d\n",BoxBoundary[3], (int)(std::round(BoxBoundary[3] - (BoxDynamic[2] + x2_max * H[1]))/H[1]));
            ...
            if ( (BoxDynamic[4] == BoxBoundary[4]) && x3_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x3_min to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[4] = %lf, dist = %d\n", BoxBoundary[4], x3_min);
            ...
            if ( (BoxDynamic[5] == BoxBoundary[5]) && (int)(std::round(BoxBoundary[5] - (BoxDynamic[4] + x3_max * H[2]))/H[2]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x3_max to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[5] = %lf, dist = %d\n", BoxBoundary[5], (int)(std::round(BoxBoundary[5] - (BoxDynamic[4] + x3_max * H[2]))/H[2]));
            ...
            if ( (BoxDynamic[6] == BoxBoundary[6]) && x4_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x4_min to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[6] = %lf, dist = %d\n", BoxBoundary[6], x4_min);
            ...
            if ( (BoxDynamic[7] == BoxBoundary[7]) && (int)(std::round(BoxBoundary[7] - (BoxDynamic[6] + x4_max * H[3]))/H[3]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[Diosi4d] x4_max to boundary is less than 3\n");
                log->log("[Diosi4d] BoxBoundary[7] = %lf, dist = %d\n", BoxBoundary[7], (int)(std::round(BoxBoundary[7] - (BoxDynamic[6] + x4_max * H[3]))/H[3]));
        ...
        {   
            ...
            if ( !QUIET )  {
                ...
                log->log("[Diosi4d] Step: %d, Elapsed time: %lf sec\n", tt + 1, t_0_elapsed);
                ta_size = 0;
                ...
                tb_size = TB.size();
                log->log("[Diosi4d] TA size = %lu, TB size = %lu\n", ta_size, tb_size);
                log->log("[Diosi4d] BoxPosition: [%.2f,%.2f][%.2f,%.2f][%.2f,%.2f][%.2f,%.2f]\n",BoxDynamic[0],BoxDynamic[1],BoxDynamic[2],BoxDynamic[3],BoxDynamic[4],BoxDynamic[5],BoxDynamic[6],BoxDynamic[7]);
                log->log("[Diosi4d] BoxShape: [%d,%d,%d,%d]\n",BoxShapeFine[0],BoxShapeFine[1],BoxShapeFine[2],BoxShapeFine[3]);
                log->log("[Diosi4d] TA Range [%d, %d][%d, %d][%d, %d][%d, %d]\n", x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max);
                log->log("[Diosi4d] TA / total grids = %lf\n", ( ta_size * 1.0 ) / GRIDS_TOT);
                log->log("[Diosi4d] ExCount = %d ExLimit = %d\n", Excount, ExLimit);
                log->log("\n........................................................\n\n");
            }
        }          
    } // Time iteration 
    ...
    log->log("[Diosi4d] Evolve done.\n");
}
/* =============================================================================== */

/* DS4DPOT_DW1 */

inline double Diosi4d::Wavefunction_DW1(double x1, double x2, double x3, double x4)
{
...
inline double Diosi4d::Wavefunction_DW2(double x1, double x2, double x3, double x4)
{
    return PIHBSQ_INV * exp(-2.0 * (A[0]*pow(x1-Wave0[0],2)+A[1]*pow(x2-Wave0[1],2))) * exp( -0.5 * HBSQ_INV * (pow(x3-Wave0[2],2)/A[2]+pow(x4-Wave0[3],2)/A[3]) );
}
/* ------------------------------------------------------------------------------- */

inline double Diosi4d::Potential_DW2(double x1, double x2)
{
...
inline double Diosi4d::Vx1_DW2(double x1, double x2)
{
...
inline double Diosi4d::Vx2_DW2(double x1, double x2)
{
    return k0 * x2 * (1.0 - sigma * exp(-lambda * x1 * x1));
    //return k0 * x2;
}
...
inline double Diosi4d::Vxxx1_DW2(double x1, double x2)
{
...
inline double Diosi4d::Vxxx2_DW2(double x1, double x2)
{
    return 0.0;
}
/* =============================================================================== */
...
inline double Diosi4d::Wavefunction_DW3(double x1, double x2, double x3, double x4)
{
    return PIHBSQ_INV * exp(-2.0 * (A[0]*pow(x1-Wave0[0],2)+A[1]*pow(x2-Wave0[1],2))) * exp( -0.5 * HBSQ_INV * (pow(x3-Wave0[2],2)/A[2]+pow(x4-Wave0[3],2)/A[3]) );
}
/* ------------------------------------------------------------------------------- */

inline double Diosi4d::Potential_DW3(double x1, double x2)
{
    return 3.0 * x1 * x1 * x1 * x1 - 8.0 * x1 * x1 + x2 * x2 - 0.1 * x1 * x2;    
}
/* ------------------------------------------------------------------------------- */

inline double Diosi4d::Vx1_DW3(double x1, double x2)
{
    return 12.0 * x1 * x1 * x1 - 16.0 * x1 - 0.1 * x2;
}
/* ------------------------------------------------------------------------------- */

inline double Diosi4d::Vx2_DW3(double x1, double x2)
{
    return 2.0 * x2 - 0.1 * x1;
}
/* ------------------------------------------------------------------------------- */

inline double Diosi4d::Vxxx1_DW3(double x1, double x2)
{
    return 72.0 * x1;
}
/* ------------------------------------------------------------------------------- */

inline double Diosi4d::Vxxx2_DW3(double x1, double x2)
{
    return 0.0;
}
/* ------------------------------------------------------------------------------- */

VectorXi Diosi4d::IdxToGrid(unsigned long int idx)
{
    int x4 = (int)( idx % M3 );
...
inline unsigned long int Diosi4d::GridToIdx(int x1, int x2, int x3, int x4)
{
