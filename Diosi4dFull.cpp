...
void Diosi4d::Evolve()
{
    ...
    log->log("[Diosi4d] Gamma = %.14e\n",gamma);
    log->log("[Diosi4d] Omega = %.14e\n",omega);
    log->log("[Diosi4d] Dqq = %.14e\n",Dqq);
    log->log("[Diosi4d] Dpq = %.14e\n",Dpq);
    ...
    log->log("[Diosi4d] Fine grid size = (");
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[Diosi4d] Elapsed time (Wavefunction normalization) = %lf sec\n\n", t_1_elapsed); 
    ...
    for (unsigned long int tt = 0; tt < (unsigned long int)(TIME / kk); tt ++)
    {
        ...
        {   
            ...
            if ( !QUIET )  {
                ...
                log->log("[Diosi4d] Step: %lu, Elapsed time: %lf sec\n", tt + 1, t_0_elapsed);
