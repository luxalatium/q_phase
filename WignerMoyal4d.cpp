...
//  QTR
//
//  Created by Albert Lu on 12/18/19.
//  alu@tacc.utexas.edu
//
...
WignerMoyal4d::WignerMoyal4d(class QTR *q)
{
    qtr = q;
    err = qtr->error;
    log = qtr->log;
...
WignerMoyal4d::~WignerMoyal4d()
{     
    return;
}
/* ------------------------------------------------------------------------------- */

void WignerMoyal4d::init()
{
    log->log("\n\n[WignerMoyal4d] INIT starts ...\n");
    log->log("\n\n[WignerMoyal4d] Truncated Grid\n");
    log->log("\n\n[WignerMoyal4d] Potential type: %s\n", POTNAME);
    ...
    log->log("[WignerMoyal4d] Set skin thickness to %d.\n\n",skin);

    log->log("[WignerMoyal4d] INIT done.\n\n");
}
/* ------------------------------------------------------------------------------- */

void WignerMoyal4d::Evolve()
{
    ...
    log->log("[WignerMoyal4d] Evolve starts ...\n");
    ...
    log->log("[WignerMoyal4d] nneigh = %d\n", nneigh); 

    // Compute coarse grid shape
    t_1_begin = omp_get_wtime();
    log->log("[WignerMoyal4d] Coarse grid size = (");
    ...
    if ( x1_min==0 || x2_min==0 || x3_min==0 || x4_min==0 || x1_max==BoxShapeCoarse[0]-1 || x2_max==BoxShapeCoarse[1]-1 || x3_max==BoxShapeCoarse[2]-1 || x4_max==BoxShapeCoarse[3]-1 )  {
        log->log("[WignerMoyal4d] Coarse TA range touches boundary");
        exit(1);
    }
    ...
    log->log("[WignerMoyal4d] Initital fine grid size = (");
    ...
    if ( !QUIET && TIMING ) log->log("[WignerMoyal4d] Elapsed time (make bounding box) = %lf sec\n\n", t_1_elapsed); 

    // Initialize containers
    t_0_begin = omp_get_wtime();

    log->log("[WignerMoyal4d] Initializing containers ...\n");
    ...
    for (unsigned long int  i = 0; i < O1; i++)  {
        ...
        if (!TAMask[i])
            log->log("[WignerMoyal4d] Memory allocation %s failed\n", "TAMask");
    }
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if ( !QUIET && TIMING ) log->log("[WignerMoyal4d] Elapsed time (initializing containers) = %lf sec\n\n", t_1_elapsed); 

    // .........................................................................................

    // Initialize wavefunction
    t_1_begin = omp_get_wtime();
    log->log("[WignerMoyal4d] Initializing wavefunction ...\n");  
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[WignerMoyal4d] Elapsed time (initializing wavefunction) = %lf sec\n\n", t_1_elapsed); 
    ...
    log->log("[WignerMoyal4d] Normalization factor = %.16e\n",norm);
    norm = 1.0 / norm;
    ...
    if ( isCorr )  {
        ...
        log->log("[WignerMoyal4d] corr_0 = %.16e\n",corr_0);
        log->log("[WignerMoyal4d] Time %lf, Corr = %.16e\n",0.0,1.0);

        t_1_end = omp_get_wtime();
        t_1_elapsed = t_1_end - t_1_begin;
        if (!QUIET && TIMING) log->log("[WignerMoyal4d] Elapsed time (initial density) = %lf sec\n\n", t_1_elapsed); 
    }
    ...
    log->log("[WignerMoyal4d] Initial truncation ...\n");
    ...
    log->log("[WignerMoyal4d] TA size = %lu\n", ta_size);
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[WignerMoyal4d] Elapsed time (truncation) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[WignerMoyal4d] Elapsed time (TA range) = %lf sec\n\n", t_1_elapsed); 
    ...
    tmpVec.clear();
    tb_size = TB.size();
    log->log("[WignerMoyal4d] TB size = %d\n", tb_size);
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[WignerMoyal4d] Elapsed time (TB) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING) log->log("[WignerMoyal4d] Elapsed time (TA expansion) = %lf sec\n\n", t_1_elapsed); 
    ...
    log->log("[WignerMoyal4d] TA size = %lu, TB size = %lu\n", ta_size, tb_size);

    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    if (!QUIET && TIMING)  log->log("[WignerMoyal4d] Elapsed time (TA,TB range) = %lf sec\n\n", t_1_elapsed);
    // .........................................................................................

    // Time iteration 

    log->log("=======================================================\n\n"); 
    log->log("[WignerMoyal4d] Time interation starts ...\n"); 
    log->log("[WignerMoyal4d] Number of steps = %d\n\n", (int)(TIME / kk)); 
    log->log("=======================================================\n\n"); 
    ...
    for (unsigned long int tt = 0; tt < (unsigned long int)(TIME / kk); tt ++)
    {
        ...
        if ( (tt + 1) % PERIOD == 0 )
        {
            ...
            if (isTrans)  {
                ...
                log->log("[WignerMoyal4d] trans_x0 = %lf\n", idx_x0 * H[0] + BoxDynamic[0]);
                log->log("[WignerMoyal4d] Time %lf, Trans = %.16e\n", ( tt + 1 ) * kk, pftrans);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if (isCorr)  {
                ...
                for (int i1 = 0; i1 < ibox_shape_1; i1++)  {
                    for (int i2 = 0; i2 < ibox_shape_2; i2++)  {
                        ...
                        //log->log("c %d %d %d %d\n",i1,i2, (i1+shift_x1_0)*W1_0+(i2+shift_x2_0), (i1+shift_x1_t)*W1+(i2+shift_x2_t));
        ...
        if ( isTouchedBoundary )  {
            ...
            if ( (BoxDynamic[0] == BoxBoundary[0]) && x1_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x1_min to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[0] = %lf, dist = %d\n", BoxBoundary[0], x1_min);
                ...
                x1_min = 3;
                log->log("[WignerMoyal4d] New x1_min = %d\n", x1_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[1] == BoxBoundary[1]) && (int)(std::round(BoxBoundary[1] - (BoxDynamic[0] + x1_max * H[0]))/H[0]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x1_max to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[1] = %lf, dist = %d\n",BoxBoundary[1], (int)(std::round(BoxBoundary[1] - (BoxDynamic[0] + x1_max * H[0]))/H[0]));
                ...
                log->log("[WignerMoyal4d] New x1_max = %d\n", x1_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[2] == BoxBoundary[2]) && x2_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x2_min to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[2] = %lf, dist = %d\n", BoxBoundary[2], x2_min);
                ...
                x2_min = 3;
                log->log("[WignerMoyal4d] New x2_min = %d\n", x2_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[3] == BoxBoundary[3]) && (int)(std::round(BoxBoundary[3] - (BoxDynamic[2] + x2_max * H[1]))/H[1]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x2_max to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[3] = %lf, dist = %d\n",BoxBoundary[3], (int)(std::round(BoxBoundary[3] - (BoxDynamic[2] + x2_max * H[1]))/H[1]));
                ...
                log->log("[WignerMoyal4d] New x2_max = %d\n", x2_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[4] == BoxBoundary[4]) && x3_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x3_min to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[4] = %lf, dist = %d\n", BoxBoundary[4], x3_min);
                ...
                x3_min = 3;
                log->log("[WignerMoyal4d] New x3_min = %d\n", x3_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[5] == BoxBoundary[5]) && (int)(std::round(BoxBoundary[5] - (BoxDynamic[4] + x3_max * H[2]))/H[2]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x3_max to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[5] = %lf, dist = %d\n", BoxBoundary[5], (int)(std::round(BoxBoundary[5] - (BoxDynamic[4] + x3_max * H[2]))/H[2]));
                ...
                log->log("[WignerMoyal4d] New x3_max = %d\n", x3_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[6] == BoxBoundary[6]) && x4_min < 3 )  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x4_min to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[6] = %lf, dist = %d\n", BoxBoundary[6], x4_min);
                ...
                x4_min = 3;
                log->log("[WignerMoyal4d] New x4_min = %d\n", x4_min);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
            ...
            if ( (BoxDynamic[7] == BoxBoundary[7]) && (int)(std::round(BoxBoundary[7] - (BoxDynamic[6] + x4_max * H[3]))/H[3]) < 3)  {
                ...
                t_1_begin = omp_get_wtime();
                log->log("[WignerMoyal4d] x4_max to boundary is less than 3\n");
                log->log("[WignerMoyal4d] BoxBoundary[7] = %lf, dist = %d\n", BoxBoundary[7], (int)(std::round(BoxBoundary[7] - (BoxDynamic[6] + x4_max * H[3]))/H[3]));
                ...
                log->log("[WignerMoyal4d] New x4_max = %d\n", x4_max);
                t_1_end = omp_get_wtime();
                t_1_elapsed = t_1_end - t_1_begin; 
        ...
        {   
            ...
            if ( !QUIET )  {
                ...
                log->log("[WignerMoyal4d] Step: %d, Elapsed time: %lf sec\n", tt + 1, t_0_elapsed);
                ta_size = 0;
                ...
                tb_size = TB.size();
                log->log("[WignerMoyal4d] TA size = %lu, TB size = %lu\n", ta_size, tb_size);
                log->log("[WignerMoyal4d] BoxPosition: [%.2f,%.2f][%.2f,%.2f][%.2f,%.2f][%.2f,%.2f]\n",BoxDynamic[0],BoxDynamic[1],BoxDynamic[2],BoxDynamic[3],BoxDynamic[4],BoxDynamic[5],BoxDynamic[6],BoxDynamic[7]);
                log->log("[WignerMoyal4d] BoxShape: [%d,%d,%d,%d]\n",BoxShapeFine[0],BoxShapeFine[1],BoxShapeFine[2],BoxShapeFine[3]);
                log->log("[WignerMoyal4d] TA Range [%d, %d][%d, %d][%d, %d][%d, %d]\n", x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max);
                log->log("[WignerMoyal4d] TA / total grids = %lf\n", ( ta_size * 1.0 ) / GRIDS_TOT);
                log->log("[WignerMoyal4d] ExCount = %d ExLimit = %d\n", Excount, ExLimit);
                log->log("\n........................................................\n\n");
            }
        }          
    } // Time iteration 
    ...
    log->log("[WignerMoyal4d] Evolve done.\n");
}
/* =============================================================================== */

/* WM4DPOT_DW1 */

inline double WignerMoyal4d::Wavefunction_DW1(double x1, double x2, double x3, double x4)
{
...
inline double WignerMoyal4d::Potential_DW1(double x1, double x2)
{
    return 0.025 * (x1 * x1 - 1.0) * (x1 * x1 - 1.0) + 0.5 * k0 * (1.0 - sigma * exp(- lambda * x1 * x1)) * x2 * x2;    
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vx1_DW1(double x1, double x2)
{
    return 0.1 * x1 * (x1 * x1 - 1.0) + k0 * sigma * lambda * x1 * x2 * x2 * exp(-lambda * x1 * x1);
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vx2_DW1(double x1, double x2)
{
    return k0 * x2 * (1.0 - sigma * exp(-lambda * x1 * x1));
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vxxx1_DW1(double x1, double x2)
{
    return 0.6 * x1 + 2.0 * k0 * sigma * lambda * lambda * x1 * x2 * x2 * (2.0 * lambda * x1 * x1 - 3.0) * exp(-lambda * x1 * x1);
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vxxx2_DW1(double x1, double x2)
{
    return 0.0;
}
/* =============================================================================== */
...
inline double WignerMoyal4d::Wavefunction_DW2(double x1, double x2, double x3, double x4)
{
    return PIHBSQ_INV * exp(-2.0 * (A[0]*pow(x1-Wave0[0],2)+A[1]*pow(x2-Wave0[1],2))) * exp( -0.5 * HBSQ_INV * (pow(x3-Wave0[2],2)/A[2]+pow(x4-Wave0[3],2)/A[3]) );
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Potential_DW2(double x1, double x2)
{
...
inline double WignerMoyal4d::Vx1_DW2(double x1, double x2)
{
...
inline double WignerMoyal4d::Vx2_DW2(double x1, double x2)
{
    //return k0 * x2 * (1.0 - sigma * exp(-lambda * x1 * x1));
    return k0 * x2;
}
...
inline double WignerMoyal4d::Vxxx1_DW2(double x1, double x2)
{
...
inline double WignerMoyal4d::Vxxx2_DW2(double x1, double x2)
{
    return 0.0;
}
/* =============================================================================== */
...
inline double WignerMoyal4d::Wavefunction_DW3(double x1, double x2, double x3, double x4)
{
    return PIHBSQ_INV * exp(-2.0 * (A[0]*pow(x1-Wave0[0],2)+A[1]*pow(x2-Wave0[1],2))) * exp( -0.5 * HBSQ_INV * (pow(x3-Wave0[2],2)/A[2]+pow(x4-Wave0[3],2)/A[3]) );
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Potential_DW3(double x1, double x2)
{
    return 3.0 * x1 * x1 * x1 * x1 - 8.0 * x1 * x1 + x2 * x2 - 0.1 * x1 * x2;    
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vx1_DW3(double x1, double x2)
{
    return 12.0 * x1 * x1 * x1 - 16.0 * x1 - 0.1 * x2;
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vx2_DW3(double x1, double x2)
{
    return 2.0 * x2 - 0.1 * x1;
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vxxx1_DW3(double x1, double x2)
{
    return 72.0 * x1;
}
/* ------------------------------------------------------------------------------- */

inline double WignerMoyal4d::Vxxx2_DW3(double x1, double x2)
{
    return 0.0;
}
/* ------------------------------------------------------------------------------- */

VectorXi WignerMoyal4d::IdxToGrid(unsigned long int idx)
{
    int x4 = (int)( idx % M3 );
...
inline unsigned long int WignerMoyal4d::GridToIdx(int x1, int x2, int x3, int x4)
{
